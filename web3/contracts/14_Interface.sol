...
// 另外，未实现的函数需要加virtual，以便子合约重写。拿我们之前的插入排序合约为例，如果我们还没想好具体怎么实现插入排序函数，那么可以把合约标为abstract，之后让别人补写上。

abstract contract IsertionSort {
...
// 1. 合约里每个函数的函数签名函数名(每个参数类型），以及bytes4选择器（通过对函数签名哈希运算后取前 4 字节获得）。
// 2. 接口id
